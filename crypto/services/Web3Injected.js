...
export const Web3Injected = (type) => {
  let WEB3_LINK = `https://mainnet.infura.io/v3/${BlocksoftExternalSettings.getStatic(
    'ETH_INFURA'
  )}`;
  ...
  if (
    ...
    WEB3_LINK = `https://ropsten.infura.io/v3/${BlocksoftExternalSettings.getStatic(
      'ETH_INFURA'
    )}`;
    MAIN_CHAIN_ID = 3;
  } else if (type === 4 || type === 'rinkeby' || type === 'ETH_RINKEBY') {
    MAIN_CURRENCY_CODE = 'ETH_RINKEBY';
    WEB3_LINK = `https://rinkeby.infura.io/v3/${BlocksoftExternalSettings.getStatic(
      'ETH_INFURA'
    )}`;
    MAIN_CHAIN_ID = 4;

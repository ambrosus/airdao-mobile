...
class BlocksoftTransactions {
  ...
  async getTransactions(data, source = '') {
    ...
    if (!this._processor[currencyCode]) {
      ...
      this._processor[currencyCode] =
        AirDAODispatcher.getScannerProcessor(currencyCode);
    }
    let resultData = [];
    try {
      resultData = await this._processor[
        currencyCode
      ].getTransactionsBlockchain(data, source);
    } catch (e) {
      e.code = 'ERROR_SYSTEM';
  ...
  async getTransactionsPending(data, source = '') {
    ...
    if (
      typeof this._processor[currencyCode].getTransactionsPendingBlockchain ===
      'undefined'
    ) {
      return false;
    }
    ...
    try {
      resultData = await this._processor[
        currencyCode
      ].getTransactionsPendingBlockchain(data, source, false);
    } catch (e) {
  ...
  async resetTransactionsPending(data, source = '') {
    ...
    if (
      typeof this._processor[currencyCode]
        .resetTransactionsPendingBlockchain === 'undefined'
    ) {
      return false;
    }
    let resultData = false;
    try {
      resultData = await this._processor[
        currencyCode
      ].resetTransactionsPendingBlockchain(data, source, false);
    } catch (e) {
  ...
  async getAddresses(data, source = '') {
    ...
    let resultData = [];
    try {
      resultData = await this._processor[currencyCode].getAddressesBlockchain(
        data,
        source
      );
    } catch (e) {

...
export class BlocksoftKeysStorage {
  ...
  async _getKeyValue(key) {
    const res = await Keychain.getInternetCredentials(
      this._serviceName + '_' + key,
      {
  ...
  async _setKeyValue(key, pub, priv = false) {
    ...
    let res = false;
    try {
      res = await Keychain.setInternetCredentials(
        this._serviceName + '_' + key,
        pub,
        priv,
        {
  ...
  async getOldSelectedWallet() {
    ...
    if (
      ...
      !this._serviceWallets[this.publicSelectedWallet]
    ) {
  ...
  async isMnemonicAlreadySaved(newMnemonic) {
    await this._init();
    if (
      typeof this._serviceWallets[newMnemonic.hash] === 'undefined' ||
      !this._serviceWallets[newMnemonic.hash]
    ) {
      return false;
    }
  ...
  async saveMnemonic(newMnemonic) {
    ...
    await this._setKeyValue(
      'wallet_' + this._serviceWalletsCounter,
      unique,
    ...
    this._serviceWallets[this._serviceWalletsCounter - 1] =
      newMnemonic.mnemonic;
  ...
  getAddressCacheKey(walletHash, discoverPath, currencyCode) {
    ...
    if (
      ...
      settings.tokenBlockchain !== 'BITCOIN'
    ) {
      return (
        walletHash + '_' + discoverPath + '_' + settings.addressCurrencyCode
      );
    }

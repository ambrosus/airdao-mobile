...
class BlocksoftBalances {
  ...
  setAddress(address) {
    this._data.address =
      typeof address.trim !== 'undefined' ? address.trim() : address;
    return this;
  }
  ...
  async getBalance(source) {
    ...
    if (
      currencyCode === 'BTC' &&
      this._data.address.toString().substr(0, 1) === 'm'
    ) {
      throw new Error(
        'plz check btc address as its testnet and mainnet is selected'
      );
    }
    let res;
    try {
      res = await this._processor[currencyCode].getBalanceBlockchain(
        this._data.address,
        this._data.jsonData,
  ...
  async getResources(source) {
    ...
    let res;
    try {
      res = await this._processor[currencyCode].getResourcesBlockchain(
        this._data.address,
        this._data.jsonData,
        this._data.walletHash,
        source
  ...
  async isMultisig(source) {
    ...
    let res;
    try {
      res = await this._processor[currencyCode].isMultisigBlockchain(
        this._data.address,
        this._data.jsonData,
        this._data.walletHash,
        source

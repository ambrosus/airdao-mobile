...
export default {
  secret_spend_key_to_words(secretSpendKeyBufferOrHex, walletHash) {
    ...
    for (var i = 0; i < 32; i += 4) {
      ...
      for (var j = 3; j >= 0; j--) {
        ...
        if (typeof buff[i + j] === 'undefined') {
          throw new Error(
            'XMR word for wallet ' +
              walletHash +
              ' need to be rechecked as buff is too low'
          );
        }
      }
      ...
      var w2 =
        (((w0 / MoneroDict.monero_words_english.length) | 0) + w1) %
        MoneroDict.monero_words_english.length;
      var w3 =
        (((((w0 / MoneroDict.monero_words_english.length) | 0) /
          MoneroDict.monero_words_english.length) |
          0) +
          w2) %
        MoneroDict.monero_words_english.length;
      ...
      forChecksum += MoneroDict.monero_words_english[w1].substring(
        0,
        MoneroDict.monero_words_english_prefix_len
      );
      forChecksum += MoneroDict.monero_words_english[w2].substring(
        0,
        MoneroDict.monero_words_english_prefix_len
      );
      forChecksum += MoneroDict.monero_words_english[w3].substring(
        0,
        MoneroDict.monero_words_english_prefix_len
      );
    }

...
export default class XmrSendProvider {
  ...
  public async send(params: any): Promise<any> {
    ...
    if (
      typeof this._link !== 'boolean' &&
      this._link?.indexOf('mymonero.com') !== -1
    ) {
      try {
        ...
        BlocksoftCryptoLog.log(
          'XmrSendProvider mymonero.com node ' + this._link,
          res.data
        );
        return res.data;
      } catch (e: any) {
      ...
      const resNode = await BlocksoftAxios.post(
      ...
      BlocksoftCryptoLog.log(
        'XmrSendProvider custom node ' + this._link,
        resNode.data
      );

...
export default class XmrScannerProcessor {
  ...
  async _get(
    ...
    const discoverFor = {
      ...
      derivationIndex:
        typeof additionalData.derivationIndex !== 'undefined'
          ? additionalData.derivationIndex
          : 0
    };
    ...
    const linkParams = { address, view_key: viewKey };
    ...
    if (typeof res.data.spent_outputs === 'undefined') {
      throw new Error(
        'XMR XmrScannerProcessor._get nothing loaded for address ' + link
      );
    }

    let parsed = false;
    try {
      parsed = await MoneroUtilsParser.parseAddressInfo(
        address,
        res.data,
        viewKey,
    ...
    let parsed2 = false;
    try {
      parsed2 = await MoneroUtilsParser.parseAddressTransactions(
        address,
        res2.data,
        viewKey,
        additionalData.publicSpendKey,
    ...
    if (parsed && parsed2) {
      CACHE[address] = {
        ...
        transactions:
          typeof parsed2.serialized_transactions !== 'undefined'
            ? parsed2.serialized_transactions
            : parsed2.transactions,
        balance:
          typeof parsed.total_received_String !== 'undefined'
            ? BlocksoftUtils.diff(
                parsed.total_received_String,
                parsed.total_sent_String
              )
            : BlocksoftUtils.diff(parsed.total_received, parsed.total_sent),

  ...
  implements BlocksoftBlockchainTypes.TransferProcessor
{
  ...
  async getTransferAllBalance(
    ...
    try {
      ...
      if (
        ...
        typeof res?.data?.value?.base !== 'undefined'
      ) {
        if (res?.data?.value?.base?.address?.address !== address) {
          throw new Error(
            'wrong value address ' + res?.data?.value?.base?.address?.address
          );
        }
    ...
    const amount = BlocksoftUtils.diff(
      balance,
      fees.fees[0].feeForTx
    ).toString();
  ...
  async sendTx(
    ...
    if (
      ...
      data.addressTo !== 'STAKE' &&
      data.addressTo.indexOf('UNSTAKE') === -1
    ) {
      if (typeof uiData?.selectedFee?.bseOrderId === 'undefined') {
        const balance = await BlocksoftBalances.setCurrencyCode('SOL')
          .setAddress(data.addressTo)
          .getBalance('SolSendTx');
        if (
          !balance ||
          typeof balance.balance === 'undefined' ||
          balance.balance === 0
        ) {
    ...
    try {
      ...
      if (data.addressTo.indexOf('UNSTAKE') === 0) {
        ...
        if (
          ...
          lastSeed &&
          typeof lastSeed.seed !== 'undefined' &&
          lastSeed.seed
        ) {
        ...
        for (let i = 1; i <= 10000; i++) {
          ...
          const isUsed = SolStakeUtils.checkAccountStaked(
            data.addressFrom,
            stakeAddress
          );
          if (!isUsed) {
  ...
  trxError(msg: string, addressTo: string) {
    if (msg.indexOf('insufficient funds for instruction') !== -1) {
      ...
      msg.indexOf('incorrect program id for instruction') !== -1 &&
      addressTo === 'STAKE'
    ) {

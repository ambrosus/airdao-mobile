  ...
  implements BlocksoftBlockchainTypes.TransferProcessor
{
  async getFeeRate(
    ...
    const destinationAccountInfo = await SolUtils.getAccountInfo(
      destinationAssociatedTokenAddress
    );
    let feeForTx = BlocksoftExternalSettings.getStatic('SOL_PRICE'); // ◎0.000005
    if (
      destinationAccountInfo &&
      typeof destinationAccountInfo.owner !== 'undefined' &&
      destinationAccountInfo.owner === SolUtils.getTokenProgramID()
    ) {
      // do nothing
    } else {
  ...
  async sendTx(
    ...
    const sourceAccountInfo = await SolUtils.getAccountInfo(
      sourceAssociatedTokenAddress
    );
    if (
      !sourceAccountInfo ||
      typeof sourceAccountInfo.lamports === 'undefined' ||
      sourceAccountInfo.lamports * 1 === 0
    ) {
    ...
    if (
      ...
      if (
        !destinationAccountInfo ||
        typeof destinationAccountInfo.lamports === 'undefined' ||
        destinationAccountInfo.lamports * 1 === 0
      ) {
      ...
      destinationAccountInfo = await SolUtils.getAccountInfo(
        destinationAssociatedTokenAddress
      );
      if (
        destinationAccountInfo &&
        ...
        const tmp1 = new TransactionInstruction({
          keys: [
            {
              pubkey: new PublicKey(data.addressTo),
              isSigner: false,
              isWritable: false
            }
          ],
          data: SolInstructions.encodeOwnerValidationInstruction({
        ...
        const keys = [
          {
            pubkey: new PublicKey(data.addressFrom),
            isSigner: true,
            isWritable: true
          },
          {
            pubkey: new PublicKey(destinationAssociatedTokenAddress),
            isSigner: false,
            isWritable: true
          },
          {
          ...
          {
            pubkey: new PublicKey(this._settings.tokenAddress),
            isSigner: false,
            isWritable: false
          },
          {
          ...
          {
            pubkey: new PublicKey(SolUtils.getTokenProgramID()),
            isSigner: false,
            isWritable: false
          },
          {
    ...
    try {
      ...
      const keys = [
        {
          pubkey: new PublicKey(sourceAssociatedTokenAddress),
          isSigner: false,
          isWritable: true
        },
        {
          pubkey: new PublicKey(txData.mint),
          isSigner: false,
          isWritable: false
        },
    ...
    await SolUtils.signTransaction(
      tx,

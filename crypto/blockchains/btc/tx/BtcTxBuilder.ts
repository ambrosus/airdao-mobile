  ...
  implements AirDAOBlockchainTypes.TxBuilder
{
  ...
  async _getRawTxAddInput(
    ...
    if (typeof this.keyPairBTC[input.address] === 'undefined') {
      ...
      if (input.address.indexOf(segwitPrefix) === 0) {
        ...
        input.address.indexOf(segwitCompatiblePrefix) === 0
      ) {
      ...
      try {
        this.keyPairBTC[input.address] = ECPair.fromWIF(
          result.privateKey,
        ...
        if (currencyCode === mainCurrencyCode + '_SEGWIT') {
          try {
            ...
            e.message +=
              ' in privateKey ' + currencyCode + ' Segwit signature create';
          ...
          try {
            ...
            e.message +=
              ' in privateKey ' +
              currencyCode +
              ' SegwitCompatible signature create';
            // noinspection ExceptionCaughtLocallyJS
            throw e;
          }
          if (typeof this.p2shBTC[input.address].address === 'undefined') {
            // noinspection ExceptionCaughtLocallyJS
            throw new Error(
              'not valid ' + currencyCode + ' segwit compatible p2sh'
            );
          }
    ...
    if (typeof this.p2wpkhBTC[input.address] === 'undefined') {
      ...
      txb.addInput(
        ...
        input.vout,
        nSequence,
        this.p2wpkhBTC[input.address].output
      );
    } else {
      txb.addInput(input.txid, input.vout, nSequence);
    }
  ...
  async _getRawTxSign(
    ...
    if (typeof this.p2wpkhBTC[input.address] === 'undefined') {
      ...
      txb.sign(
        i,
        this.keyPairBTC[input.address],
        this.p2shBTC[input.address].redeem.output,
        null,
        input.value * 1
      );
    }

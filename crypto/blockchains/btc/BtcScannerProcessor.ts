...
export default class BtcScannerProcessor {
  ...
  async _get(
    ...
    if (typeof res.data.balance === 'undefined') {
      throw new Error(
        this._settings.currencyCode +
          ' BtcScannerProcessor._get nothing loaded for address ' +
          link
      );
    }
  ...
  async getBalanceBlockchain(address, data, walletHash, source = '') {
    ...
    return {
      ...
      specialMark:
        typeof res.data.specialMark !== 'undefined'
          ? res.data.specialMark
          : false
    };
  }
      ...
      if (typeof data.walletPub !== 'undefined') {
        ...
        for (const scanAddress in resPub) {
          ...
          if (
            ...
            typeof tmp.data.plainAddresses === 'undefined'
          )
            continue;
  ...
  async getTransactionsBlockchain(scanData, source = '') {
    ...
    try {
      if (typeof data.walletPub !== 'undefined') {
        ...
        for (const scanAddress in resPub) {
          ...
          if (
            typeof tmp.data === 'undefined' ||
            typeof tmp.data.transactions === 'undefined'
          )
            continue;
    ...
    if (
      ...
      data &&
      typeof data.addresses !== 'undefined'
    ) {
    ...
    for (const tx of res.transactions) {
      ...
      if (transaction) {
        if (typeof uniqueTxs[transaction.transactionHash] !== 'undefined')
          continue;
  ...
  async _unifyTransaction(
    ...
    if (
      typeof showAddresses.to !== 'undefined' &&
      showAddresses.to.toLowerCase().indexOf('simple send') !== -1
    ) {

  ...
  implements AirDAOBlockchainTypes.TxInputsOutputs
{
  ...
  async getInputsOutputs(
    ...
    if (newInputAdded) {
      ...
      for (const output of res.outputs) {
        if (typeof output.isChange !== 'undefined' && output.isChange) {
          output.amount = BlocksoftUtils.add(
            output.amount,
            newInputAdded.value
          );
          changeIsFound = true;
    ...
    const tmp =
      typeof additionalData.balance !== 'undefined' && additionalData.balance
        ? { balance: additionalData.balance }
        : DaemonCache.getCacheAccountStatic(data.walletHash, 'USDT');
    ...
    if (
      res.inputs.length === 0 &&
      (!inputIsFound || newInputAdded.value === '546')
    ) {
    ...
    if (totalOuts === 0) {
      ...
      if (
        needOneOutput &&
        (res.inputs.length > 1 ||
          res.inputs[0].value * 1 >= this.DUST_FIRST_TRY * 2)
      ) {
        newRes.outputs.push({
          isUsdt: true,
  ...
  _innerResort(
    ...
    if (res.outputs[0].amount !== '0') {
      if (totalOuts > 1) {
        if (res.outputs[0].to !== addressTo) {
          throw new Error(
            'usdt addressTo is invalid1.1 ' + JSON.stringify(res.outputs)
          );
        } else if (res.outputs[1].to !== res.outputs[0].to) {
          throw new Error(
            'usdt addressTo is invalid1.2 ' + JSON.stringify(res)
          );
        }
        if (res.outputs[1].to !== res.outputs[0].to) {
          throw new Error(
            'usdt addressTo is invalid1.2 ' + JSON.stringify(res.outputs)
          );
        }
        res.outputs[1].amount = BlocksoftUtils.add(
          res.outputs[0].amount,
          res.outputs[1].amount
        ).toString();
      } else {
        if (res.outputs[0].to !== addressTo) {
          throw new Error(
            'usdt addressTo is invalid2 ' + JSON.stringify(res.outputs)
          );
        }
      ...
      if (totalOuts > 1) {
        if (res.outputs[1].to !== addressTo) {
          throw new Error(
            'usdt addressTo is invalid3 ' + JSON.stringify(res.outputs)
          );
        }
        res.outputs[1].isUsdt = true;
      } else {
        throw new Error(
          'usdt addressTo is empty ' + JSON.stringify(res.outputs)
        );
      }
    }
    const newOutputs = [];
    if (needOneOutput) {
      if (
        typeof res.outputs[2] === 'undefined' ||
        res.outputs[2].to !== addressFrom
      ) {
        newOutputs.push({
          isUsdt: true,
          amount: this.DUST_FIRST_TRY.toString(),
  ...
  _innerToUp(outputs: any, addressTo: string) {
    ...
    if (result.length === 1) {
      ...
      if (
        amount === '0' ||
        BlocksoftUtils.diff(amount, this.DUST_FIRST_TRY.toString())
          .toString()
          .indexOf('-') !== -1
      ) {
        amount = this.DUST_FIRST_TRY.toString();

...
export default class EthScannerProcessorSoul extends EthScannerProcessorErc20 {
  ...
  async _unifyTransactions(
    ...
    for (tx of result) {
      ...
      transaction.addressAmount = new BlocksoftBN(
        BlocksoftUtils.fromENumber(transaction.addressAmount)
      );
      if (
        typeof alreadyTransactions[transaction.transactionHash] !== 'undefined'
      ) {

...
export default async function (data) {
  ...
  if (
    result &&
    result.data &&
    typeof result.data.assets !== 'undefined' &&
    result.data.assets &&
    ...
    for (const tmp of result.data.assets) {
      ...
      try {
        ...
        if (
          typeof tmp.asset_contract.description !== 'undefined' &&
          tmp.asset_contract.description
        ) {
      ...
      try {
        if (
          typeof tmp.asset_contract.address !== 'undefined' &&
          tmp.asset_contract.address
        ) {
          one.contractAddress = tmp.asset_contract.address;
        }
        if (
          typeof tmp.asset_contract.schema_name !== 'undefined' &&
          tmp.asset_contract.schema_name
        ) {
      ...
      try {
        if (typeof tmp.last_sale !== 'undefined' && tmp.last_sale) {
          ...
          one.cryptoValue = BlocksoftUtils.toUnified(
            tmp.last_sale.total_price,
            tmp.last_sale.payment_token.decimals
          );
      ...
      try {
        if (typeof tmp.collection !== 'undefined') {
          collectionKey =
            tmp.collection.name + '_' + tmp.collection.payout_address;

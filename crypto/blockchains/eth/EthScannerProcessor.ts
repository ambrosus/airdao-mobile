...
export default class EthScannerProcessor extends EthBasic {
  ...
  async getBalanceBlockchain(address) {
    ...
    try {
      if (
        this._trezorServerCode &&
        this._trezorServerCode.indexOf('http') === -1
      ) {
  ...
  async getTransactionsBlockchain(scanData) {
    ...
    if (
      ...
      this._settings.currencyCode !== 'ETH_RINKEBY' &&
      this._trezorServerCode
    ) {
      try {
    ...
    if (res && typeof res.data !== 'undefined' && res.data) {
      ...
      let logTitle =
        this._settings.currencyCode +
        ' EthScannerProcessor.getTransactions oklink ';
      transactions = await this._getFromOklink(
        address,
        this._oklinkAPI,
        logTitle,
        {}
      ...
      let logTitle =
        this._settings.currencyCode +
        ' EthScannerProcessor.getTransactions etherscan ';
      transactions = await this._getFromEtherscan(
        address,
        link,
        logTitle,
        false,
      ...
      if (this._useInternal && this._etherscanApiPathInternal) {
        ...
        logTitle =
          this._settings.currencyCode +
          ' EthScannerProcessor.getTransactions etherscan forInternal';
        transactions = await this._getFromEtherscan(
          address,
          link,
          logTitle,
  ...
  async _getFromOklink(address, key, logTitle, transactions = {}) {
    ...
    if (
      typeof tmp?.data?.data === 'undefined' ||
      typeof tmp?.data?.data[0] === 'undefined' ||
      typeof tmp?.data?.data[0]?.transactionLists === 'undefined'
    ) {
      return [];
    }
  ...
  async _getFromEtherscan(
    ...
    if (
      ...
      !tmp.data ||
      typeof tmp.data.result === 'undefined'
    ) {
      return transactions;
    }
    if (typeof tmp.data.result === 'string') {
      if (tmp.data.result.indexOf('API Key') === -1) {
        throw new Error(
          'Undefined txs etherscan ' + link + ' ' + tmp.data.result
        );
      } else {
        return transactions;
      }
  ...
  async _unifyTransactions(
    ...
    const notBroadcasted = await EthRawDS.getForAddress({
      address,
    ...
    for (tx of result) {
      try {
        ...
        if (isTrezor) {
          transaction = await this._unifyTransactionTrezor(
            address,
            tx,
            isInternal
          );
        ...
        if (transaction) {
          ...
          if (
            ...
            const uniqueFrom =
              address.toLowerCase() + '_' + transaction.transactionJson.nonce;
            if (
              notBroadcasted &&
              typeof notBroadcasted[uniqueFrom] !== 'undefined' &&
  ...
  async _unifyTransactionTrezor(_address, transaction, isInternal = false) {
    ...
    const address = _address.toLowerCase();
    if (
      typeof transaction.vin[0] !== 'undefined' &&
      transaction.vin[0].addresses &&
      typeof transaction.vin[0].addresses[0] !== 'undefined'
    ) {
      fromAddress = transaction.vin[0].addresses[0].toLowerCase();
    }
    let toAddress = '';
    if (
      typeof transaction.vout[0] !== 'undefined' &&
      transaction.vout[0].addresses &&
      typeof transaction.vout[0].addresses[0] !== 'undefined'
    ) {
        ...
        for (tmp of transaction.tokenTransfers) {
          if (tmp.token.toLowerCase() === this._tokenAddress.toLowerCase()) {
            ...
            if (tmp.to === address) {
              ...
              if (
                this._delegateAddress &&
                tmp.to.toLowerCase() === this._delegateAddress.toLowerCase()
              ) {
                fee = tmp.value;
    ...
    if (typeof transaction.blockTime === 'undefined') {
      throw new Error(
        ' no transaction.blockTime error transaction data ' +
    ...
    try {
      CACHE_BLOCK_NUMBER_TO_HASH[this._mainCurrencyCode][
      ...
      if (
        confirmations > 0 &&
        transaction.blockHeight >
          CACHE_GET_MAX_BLOCK[this._mainCurrencyCode].max_block_number
      ) {
        CACHE_GET_MAX_BLOCK[this._mainCurrencyCode].max_block_number =
          transaction.blockHeight;
        CACHE_GET_MAX_BLOCK[this._mainCurrencyCode].confirmations =
          confirmations;
      }
    } catch (e) {
      throw new Error(
        e.message + ' in  CACHE_GET_MAX_BLOCK ' + this._mainCurrencyCode
      );
    }
    ...
    const tx = {
      ...
      transactionDirection:
        address.toLowerCase() === fromAddress.toLowerCase()
          ? 'outcome'
          : 'income',
      addressFrom:
        address.toLowerCase() === fromAddress.toLowerCase() ? '' : fromAddress,
      addressTo:
        address.toLowerCase() === toAddress.toLowerCase() ? '' : toAddress,
    ...
    const tx = {
      ...
      addressTo:
        address.toLowerCase() === transaction.to.toLowerCase()
          ? ''
          : transaction.to,
      addressAmount: BlocksoftPrettyNumbers.setCurrencyCode('ETH').makeUnPretty(
        transaction.amount
      ),
      transactionStatus: transactionStatus,
      contractAddress: '',
      inputValue: '',
      transactionFee: BlocksoftPrettyNumbers.setCurrencyCode(
        'ETH'
      ).makeUnPretty(transaction.txFee)
    };
    if (tx.addressFrom === '' && tx.addressTo === '') {
      tx.transactionDirection = 'self';
      tx.addressAmount = 0;
  ...
  async _unifyTransaction(_address: string, transaction, isInternal = false) {
    ...
    if (typeof transaction.L1TxOrigin !== 'undefined') {
      ...
      CACHE_BLOCK_NUMBER_TO_HASH[transaction.blockNumber] =
        transaction.blockHash;
      ...
      if (
        typeof CACHE_BLOCK_NUMBER_TO_HASH[this._mainCurrencyCode][
      ...
      transaction.confirmations =
        CACHE_GET_MAX_BLOCK[this._mainCurrencyCode].max_block_number -
        transaction.blockNumber +
        1 * CACHE_GET_MAX_BLOCK[this._mainCurrencyCode].confirmations;
    } else {
      CACHE_BLOCK_NUMBER_TO_HASH[transaction.blockNumber] =
        transaction.blockHash;
    }
    ...
    let transactionStatus = 'new';
    if (
      typeof transaction.txreceipt_status === 'undefined' ||

...
export const getSentFioRequests = async (
  fioPublicKey,
  limit = 100,
  offset = 0
) => {
...
export const getPendingFioRequests = async (
  fioPublicKey,
  limit = 100,
  offset = 0
) => {
 ...
 */
export const addCryptoPublicAddress = async ({
  fioName,
  chainCode,
  tokenCode,
  publicAddress
...
export const addCryptoPublicAddresses = async ({
  fioName,
  publicAddresses
}) => {
...
export const removeCryptoPublicAddresses = async ({
  fioName,
  publicAddresses
}) => {
  if (!publicAddresses || Object.keys(publicAddresses).length === 0)
  ...
  for await (const publicAddressesChunk of chunk(publicAddresses, 5)) {
    try {
      const { fee = 0 } = await getFioSdk().getFeeForRemovePublicAddresses(
        fioName
      );
...
export const transferTokens = async (addressTo, amount) => {
  try {
    ...
    const result = await getFioSdk().transferTokens(
      addressTo,
      amount,
      fee,
      null
...
export const recordFioObtData = async ({
  ...
  try {
    ...
    const result = await sdk.recordObtData(
      fioRequestId,
      payerFioAddress,
      payeeFioAddress,
      payerTokenPublicAddress,
...
export const getFioObtData = async (tokenCode, offset = 0, limit = 100) => {
  ...
  try {
    ...
    formatError(
      'FIO.getFioObtData ' + tokenCode + ' ' + limit + ' ' + offset,
      e
    );
  }
  return res;

...
export default class BnbScannerProcessor {
  ...
  async getBalanceBlockchain(
    address: string
  ): Promise<{ balance: string; unconfirmed: any; provider: any }> {
  ...
  async getTransactionsBlockchain(scanData) {
    ...
    if (
      scanData.account.balanceScanTime &&
      scanData.account.balanceScanTime * 1 > 0
    ) {
      linkTxs +=
    ...
    try {
      ...
      if (transaction.txType === 'TRANSFER') {
        tx = {
          ...
          addressFrom:
            transaction.fromAddr === address ? '' : transaction.fromAddr,
        ...
        transaction.txType === 'CROSS_TRANSFER_OUT' &&
        typeof transaction.data !== 'undefined'
      ) {
        const tmp = JSON.parse(transaction.data);
        if (
          typeof tmp.amount.denom === 'undefined' ||
          tmp.amount.denom !== 'BNB'
        )
          return false;
        tx = {
          transactionHash: transaction.txHash,

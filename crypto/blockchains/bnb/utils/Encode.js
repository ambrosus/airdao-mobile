...
const UVarInt = require('./UVarInt.js').UVarInt;
const EncoderHelper = require('./EncoderHelper.js');
...
var encodeArrayBinary = function encodeArrayBinary(
  fieldNum,
  arr,
  isByteLenPrefix
) {
  ...
  if (isByteLenPrefix) {
    var length = result.reduce((prev, item) => {
...
var isDefaultValue = function isDefaultValue(obj) {
  ...
  return (
    (IsJs.number(obj) && obj === 0) ||
    (IsJs.string(obj) && obj === '') ||
    (IsJs.array(obj) && obj.length === 0) ||
    (IsJs['boolean'](obj) && !obj)
  );
};

...
function VarIntFunc(signed: boolean) {
  const encodingLength = function encodingLength(n: number) {
    if (signed) n *= 2;
  ...
  const encode = function encode(n: number, buffer?: Buffer, offset?: number) {
    if (n < 0) {
      throw Error('varint value is out of bounds');
    }
    ...
    const num128 = new BN(0x80); // amino signed varint is multiplied by 2
  ...
  const decode = function decode(bytes: number[]) {
    let x = 0;
    let s = 0;
...
const UVarInt = VarIntFunc(false);
exports.UVarInt = UVarInt;
const VarInt = VarIntFunc(true);

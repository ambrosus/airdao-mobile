...
export class BnbTxSendProvider {
  async getPrepared(
    ...
    try {
      ...
      throw new Error(
        e.message + ' in BNB decodeAddress ' + JSON.stringify(data.addressFrom)
      );
    }
    try {
      ...
      throw new Error(
        e.message + ' in BNB decodeAddress ' + JSON.stringify(data.addressTo)
      );
    }

    if (
      typeof data.blockchainData !== 'undefined' &&
      typeof data.blockchainData.action !== 'undefined' &&
      data.blockchainData.action === 'BnbToSmart'
    ) {
    ...
    const memo =
      typeof data.memo === 'undefined' || !data.memo ? '' : data.memo;
    const signMsg = {
    ...
    try {
      ...
      throw new Error(
        e.message +
          ' in BNB convertObjectToSignBytes ' +

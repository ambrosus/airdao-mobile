  ...
  implements AirDAOBlockchainTypes.TransferProcessor
{
  ...
  async getTransferAllBalance(
    ...
    // @ts-ignore
    const newAmount = BlocksoftUtils.diff(
  ...
  async sendTx(
    ...
    try {
      if (typeof result.message !== 'undefined') {
        if (
          result.message.indexOf('insufficient fund') !== -1 ||
          result.message.indexOf('BNB <') !== -1
        ) {
          throw new Error('SERVER_RESPONSE_NOTHING_TO_TRANSFER');
        } else {
      ...
      if (
        ...
        typeof result[0].hash === 'undefined' ||
        typeof result[0].ok === 'undefined' ||
        !result[0].ok ||
        !result[0].hash
      ) {
        await BlocksoftCryptoLog.log(

...
export default class XvgScannerProcessor {
  ...
  async getBalanceBlockchain(address) {
    ...
    if (typeof res.data.confirmed === 'undefined') {
      throw new Error(
        'XvgScannerProcessor.getBalance nothing loaded for address ' + link
      );
    }
  ...
  async getTransactionsBlockchain(scanData, source = '') {
    ...
    let tx;
    for (tx of tmp) {
      // ASC order is important
      const tmp2 = await this._unifyTransactionStep1(address, tx, already);
      if (tmp2) {
        if (tmp2.outcoming) {
          if (
            typeof CACHE_FROM_DB[tmp2.outcoming.transactionHash + '_data'] ===
            'undefined'
          ) {
        ...
        if (tmp2.incoming) {
          if (
            typeof CACHE_FROM_DB[tmp2.incoming.transactionHash + '_data'] ===
            'undefined'
          ) {
            tmp2.incoming = await this._unifyTransactionStep2(
              address,
              tmp2.incoming
            );
            if (tmp2.incoming) {
  ...
  async _unifyTransactionStep2(address, transaction) {
    ...
    if (
      typeof CACHE_FROM_DB[transaction.transactionHash + '_coins'] !==
      'undefined'
    ) {
    ...
    if (transaction.blockConfirmations < 0)
      transaction.blockConfirmations = transaction.blockConfirmations * -1;
  ...
  async _unifyTransactionStep1(address, transaction, already) {
    if (transaction.chain !== 'XVG' || transaction.network !== 'mainnet')
      return false;
    const res = { incoming: false, outcoming: false };
    if (
      transaction.spentTxid &&
      typeof already[transaction.spentTxid] === 'undefined'
    ) {
      res.outcoming = {
    ...
    if (
      ...
      transaction.mintTxid !== transaction.spentTxid &&
      typeof already[transaction.mintTxid] === 'undefined'
    ) {
      res.incoming = {

  ...
  async getBalanceBlockchain(address) {
    ...
    try {
      ...
      if (
        ...
        res.data &&
        typeof res.data.confirmed !== 'undefined'
      ) {
  ...
  async _unifyTransaction(address, transaction) {
    try {
      const tx = {
        ...
        addressAmount: 0,
        transactionStatus:
          transaction.confirmations > this._blocksToConfirm ? 'success' : 'new',
        transactionFee: '0'
      };
      ...
      if (!othersSumIn && !othersSumOut) {
        ...
        tx.transactionFee = BlocksoftPrettyNumbers.setCurrencyCode(
          'BSV'
        ).makeUnPretty(BlocksoftUtils.diff(mySumOut, mySumIn));
      } else if (!othersSumIn) {
        tx.transactionDirection = 'outcome';
        tx.addressTo = othersAddressOut;
        const amount = BlocksoftUtils.diff(othersSumOut, othersSumIn);
        const fee = BlocksoftUtils.diff(
          mySumOut + othersSumOut,
          mySumIn + othersSumIn
        );
        tx.addressAmount =
          BlocksoftPrettyNumbers.setCurrencyCode('BSV').makeUnPretty(amount);
        tx.transactionFee =
          BlocksoftPrettyNumbers.setCurrencyCode('BSV').makeUnPretty(fee);
      } else {

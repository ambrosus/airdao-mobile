...
class DogeRawDS {
    ...
    try {
      ...
      for (const row of result.array) {
        try {
          ...
          let broad;
          try {
            broad = await BlocksoftTransfer.sendRawTx(
              data,
              row.transactionRaw,
              typeof transactionLog !== 'undefined' &&
                transactionLog &&
            ...
            if (config.debug.cryptoErrors) {
              const dbTx = await Database.query(
                `SELECT * FROM transactions WHERE transaction_hash='${row.transactionHash}'`
              );
              if (config.debug.cryptoErrors) {
                console.log(
                  'DogeRawDS.getForAddress send error ' + e.message,
            ...
            if (
              ...
              e.message.indexOf('missing-inputs') !== -1 ||
              e.message.indexOf('insufficient fee') !== -1
            ) {
  ...
  async saveRaw(data) {
    ...
    if (typeof data.transactionLog !== 'undefined' && data.transactionLog) {
      prepared[0].transaction_log = Database.escapeString(

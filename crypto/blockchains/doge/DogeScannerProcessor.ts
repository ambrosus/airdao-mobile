...
const CACHE: { [key: string]: unknown } = {}; //TODO fix type
...
export default class DogeScannerProcessor {
  ...
  _addressesForFind(address: string, jsonData = {}) {
    return [address];
  }

  private async _get(address: string, jsonData): Promise<boolean> {
    ...
    if (
      ...
      this._settings['currencyCode'] === 'LTC' ||
      this._settings['currencyCode'] === 'BSV' ||
      this._settings['currencyCode'] === 'BCH'
    ) {
      link =
        PROXY_TXS +
        '?address=' +
    ...
    if (typeof res.data.balance === 'undefined') {
      throw new Error(
        this._settings.currencyCode +
          ' DogeScannerProcessor._get nothing loaded for address ' +
          link
      );
    }
    CACHE[address] = {
  ...
  async getTransactionsBlockchain(scanData: {
    ...
    additional: unknown; // TODO fix type
  }) {
    ...
    const notBroadcasted = await DogeRawDS.getForAddress({
      address,
    ...
    for (const tx of res.transactions) {
      ...
      if (transaction) {
        ...
        if (
          ...
          const uniqueFrom =
            address.toLowerCase() + '_' + transaction.transactionHash;
          if (
            notBroadcasted &&

  ...
  implements AirDAOBlockchainTypes.UnspentsProvider
{
  ...
  async getUnspents(
    ...
    this._trezorServer = await BlocksoftExternalSettings.getTrezorServer(
      ...
      'DOGE.Unspents.getUnspents'
    );
    let link = BlocksoftExternalSettings.getStatic(
      this._trezorServerCode + '_UNSPENDS_LINK'
    );
    if (!link || link === '') {
    ...
    for (const unspent of saved) {
      ...
      try {
        ...
        if (res2 && typeof res2.data !== 'undefined' && res2.data) {
          // @ts-ignore
          if (
            typeof res2.data.confirmations !== 'undefined' &&
            res2.data.confirmations * 1 > 0
          ) {
            // @ts-ignore
          ...
          if (
            recheckInputs &&
            typeof res2.data.vout !== 'undefined' &&
            res2.data.vout
          ) {
            // @ts-ignore
            BlocksoftCryptoLog.log(

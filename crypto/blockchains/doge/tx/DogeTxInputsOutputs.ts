  ...
  implements AirDAOBlockchainTypes.TxInputsOutputs
{
  ...
  constructor(
    ...
    this._minOutputDust = BlocksoftUtils.fromUnified(
      this._builderSettings.minOutputDustReadable,
      settings.decimals
    ); // output amount that will be considered as "dust" so we dont need it
    this._minChangeDust = BlocksoftUtils.fromUnified(
      this._builderSettings.minChangeDustReadable,
      settings.decimals
    ); // change amount that will be considered as "dust" so we dont need it
  }
  ...
  async _coinSelect(
    ...
    for (const unspent of unspents) {
      ...
      if (
        typeof unspent.address !== 'undefined' &&
        unspent.address.indexOf(segwitPrefix) === 0
      ) {
        input.isSegwit = true;
    ...
    const targets = this._coinSelectTargets(
      data,
      unspents,
      feeForByte,
      multiAddress,
    ...
    if (changeIsNeeded) {
      // @ts-ignore
      if (
        this._builderSettings.changeTogether &&
        typeof formatted.outputs[0] !== 'undefined' &&
        addressForChange === data.addressTo &&
        addressForChange === formatted.outputs[0].to
      ) {
        // @ts-ignore
  ...
  async _getInputsOutputs(
    ...
    if (typeof data.addressTo === 'undefined') {
      throw new Error(
        'DogeTxInputsOutputs.getInputsOutputs requires addressTo'
      );
    }
    ...
    for (const unspent of unspents) {
      if (
        ...
        unspent.txid === data.transactionSpeedUp
      ) {
      ...
      if (unspent.isRequired) {
        ...
        if (
          unspent.isRequired &&
          typeof isRequired[unspent.txid] === 'undefined'
        ) {
          isRequired[unspent.txid] = unspent;
        }
      } else {
    ...
    if (
      typeof feeToCount.feeForByte !== 'undefined' &&
      feeToCount.feeForByte !== 'none'
    ) {
      const result = await this._coinSelect(
        data,
        filteredUnspents,
        feeToCount.feeForByte,
        multiAddress,
        subtitle
      );
    ...
    let msg =
      ...
      totalBalanceBN.get() +
      ' = ' +
      BlocksoftUtils.toUnified(totalBalanceBN.get(), this._settings.decimals) +
      ' for wishedAmount ' +
      wishedAmountBN.get() +
      ' = ' +
      BlocksoftUtils.toUnified(wishedAmountBN.get(), this._settings.decimals);
    ...
    if (typeof feeToCount.feeForAll === 'undefined') {
      ...
      if (
        data.isTransferAll &&
        typeof feeToCount.feeForAllInputs !== 'undefined' &&
        feeToCount.feeForAllInputs * 1 > 0
      ) {
        feeToCount.feeForAll = BlocksoftUtils.mul(
          feeToCount.feeForAll,
          Math.ceil(filteredUnspents.length / feeToCount.feeForAllInputs)
        );
        wishedAmountBN = new BlocksoftBN(
          BlocksoftUtils.diff(totalBalanceBN, feeToCount.feeForAll)
        );
      ...
      msg +=
        ...
        feeToCount.feeForAll +
        ' = ' +
        BlocksoftUtils.toUnified(
          feeToCount.feeForAll.toString(),
    ...
    if (autocalculateFee) {
      ...
      msg +=
        ' auto => ' +
        BlocksoftUtils.toUnified(autoFeeBN.get(), this._settings.decimals);
    } else {
    ...
    for (let i = 0; i < ic; i++) {
      if (!data.isTransferAll) {
        const tmp = new BlocksoftBN(wishedAmountWithFeeBN).diff(
          inputsBalanceBN
        );
        if (tmp.lessThanZero()) {
          msg +=
            ' finished by collectedAmount ' +
            inputsBalanceBN.get() +
            ' = ' +
            BlocksoftUtils.toUnified(
              inputsBalanceBN.get(),
          ...
          msg +=
            ' on wishedAmountWithFee ' +
            wishedAmountWithFeeBN.get() +
            ' = ' +
            BlocksoftUtils.toUnified(
              wishedAmountWithFeeBN.get(),
              this._settings.decimals
      ...
      if (autocalculateFee) {
        ...
        if (
          typeof unspent.address !== 'undefined' &&
          unspent.address &&
          unspent.address.toString().indexOf('bc1') === 0
        ) {
    ...
    const leftForChangeDiff = new BlocksoftBN(inputsBalanceBN).diff(
      wishedAmountWithFeeBN
    );

    if (leftForChangeDiff.lessThanZero()) {
      if (autocalculateFee) {
        const newData = JSON.parse(JSON.stringify(data));
        const autoFeeLimit = BlocksoftUtils.fromUnified(
          feeToCount.autoFeeLimitReadable,
          this._settings.decimals
        );
        ...
        const res = await this._getInputsOutputs(
          newData,
          unspents,
          { feeForAll: recountWithFee },
          additionalData,
          subtitle +
            ' notEnough1 leftForChangeDiff ' +
            leftForChangeDiff.toString() +
            ' //// '
        );
      ...
      // no change
      msg +=
        ' will transfer all but later will RECHECK as change ' +
        leftForChangeDiff.toString();
      return {
        inputs,
        outputs,
    ...
    const changeDiff = new BlocksoftBN(leftForChangeDiff).diff(
      this._minChangeDust
    );
    if (changeDiff.lessThanZero()) {
      // no change
      msg +=
        ' will transfer all as change ' +
        leftForChangeDiff.toString() +
        ' - dust = ' +
        changeDiff.toString();
      return {

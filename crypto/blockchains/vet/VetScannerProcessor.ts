...
export default class VetScannerProcessor {
  ...
  private async _get(_address: string) {
  ...
  > {
    ...
    try {
      const hex =
        this._settings.currencyCode === 'VET' ? res.balance : res.energy;
    ...
    if (this._settings.currencyCode === 'VET') {
      try {
        ...
        if (
          ...
          typeof resultNumber.data.number !== 'undefined'
        ) {
      ...
      if (!result.data || result.data.length === 0) return false;
      const transactions = await this._unifyTransactionsToken(
        address,
        result.data
      );
      AirDAOCryptoLog.log(
  ...
  private async _unifyTransactionsToken(
    address: string,
    result: any[]
  ): Promise<any[]> {
    const transactions = [];
  ...
  private async _unifyTransaction(
    ...
    const diffSeconds = Math.round(
      (now - transaction.meta.blockTimestamp) / 1000
    );
  ...
  private async _unifyTransactionToken(
    address: string,
    transaction: any
  ): Promise<any | null> {
    /**
    ...
    const addressFrom =
      typeof transaction.topics[1] !== 'undefined'
        ? transaction.topics[1].replace('0x000000000000000000000000', '0x')
        : '';
    const addressTo =
      typeof transaction.topics[2] !== 'undefined'
        ? transaction.topics[2].replace('0x000000000000000000000000', '0x')
        : '';
    const tx = {
      transactionHash: transaction.meta.txID,
      blockHash: transaction.meta.blockID,
      ...
      blockConfirmations,
      transactionDirection:
        addressFrom === address.toLowerCase() ? 'outcome' : 'income',

  ...
  async _unifyTransaction(scanData, transaction, tokenName) {
    ...
    if (typeof transaction.contractData.amount === 'undefined') {
      if (
        ...
        typeof transaction.contractType !== 'undefined' &&
        transaction.contractType === 4
      ) {
        // no vote tx
        return false;
        addressAmount = BlocksoftPrettyNumbers.setCurrencyCode(
          'TRX'
        ).makeUnPretty(transaction.amount);
        addressFrom = transaction.ownerAddress;
        transactionDirection = 'vote';
        transactionFilterType = TransactionFilterTypeDict.STAKE;
      } else {
        if (
          transaction.contractType === 11 ||
          transaction.contractType === 4 ||
          transaction.contractType === 13
        ) {

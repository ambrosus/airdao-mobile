...
export function useBalance(market: MarketType, balance: string) {
  const { bnBalance, onChangeAmountToBuy } = useKosmosMarketsContextSelector();
  ...
  const calculateMaximumAvailableAmount = useCallback(() => {
    ...
    onChangeAmountToBuy(
      formatDecimals(newAmount, quoteToken?.contractAddress, tokens).toString()
    );
  }, [
    balance,
    bnBalance,

  ...
  updateAccountListDaemon = async (params) => {
    ...
    try {
      ...
      for (tmpCurrency of tmps) {
        ...
        if (
          tmpCurrency.currencyCode === 'BTC' &&
          typeof walletPub[tmpCurrency.walletHash] !== 'undefined'
        ) {
          if (
            typeof reformatted[tmpCurrency.walletHash][
              tmpCurrency.currencyCode
            ] !== 'undefined'
          ) {
            if (
              reformatted[tmpCurrency.walletHash][
              ...
              reformatted[tmpCurrency.walletHash][
                tmpCurrency.currencyCode
              ].address = tmpCurrency.address;
              reformatted[tmpCurrency.walletHash][
                tmpCurrency.currencyCode
              ].derivationPath = tmpCurrency.derivationPath;
            }
            reformatted[tmpCurrency.walletHash][
              tmpCurrency.currencyCode
            ].walletPubs = walletPub[tmpCurrency.walletHash];
          ...
          reformatted[tmpCurrency.walletHash][tmpCurrency.currencyCode] =
            tmpCurrency;
          ...
          for (key of keys) {
            ...
            if (tmpBalanceScanTime > 0) {
              if (pub[key].balanceScanTime * 1 > 0) {
                tmpBalanceScanTime = Math.min(
                  tmpBalanceScanTime,
                  pub[key].balanceScanTime
                );
              } else {
            ...
            if (tmpTransactionsScanTime > 0) {
              if (pub[key].transactionsScanTime * 1 > 0) {
                tmpTransactionsScanTime = Math.max(
                  tmpTransactionsScanTime,
                  pub[key].transactionsScanTime
                );
              }
            } else {
            ...
            if (
              pub[key].balanceScanError &&
              pub[key].balanceScanError !== '' &&
              pub[key].balanceScanError !== 'null'
            ) {
            ...
            if (pub[key].unconfirmed) {
              try {
                ...
                Log.errDaemon(
                  'UpdateAccountListDaemon error on tmpUnconfirmed ' + e.message
                );
              }
            }
          }

          const badAddresses = await accountDS.getAccountData({
            derivationPath: 'm/49quote/0quote/0/1/0'
          });
          if (badAddresses) {
            let bad;
            for (bad of badAddresses) {
              if (bad.balance) {
                try {
                  ...
                  Log.errDaemon(
                    'UpdateAccountListDaemon error on tmpBalance ' + e.message
                  );
                }
              }
              ...
              if (bad.unconfirmed) {
                try {
                  ...
                  Log.errDaemon(
                    'UpdateAccountListDaemon error on tmpUnconfirmed2 ' +
                      e.message
                  );
                }
              }
          ...
          reformatted[tmpCurrency.walletHash][
            tmpCurrency.currencyCode
          ].balanceAddingLog = tmpBalanceLog;
          reformatted[tmpCurrency.walletHash][
            tmpCurrency.currencyCode
          ].balanceScanTime =
            tmpBalanceScanTime > tmpTransactionsScanTime
              ? tmpBalanceScanTime
              : tmpTransactionsScanTime;
          reformatted[tmpCurrency.walletHash][
            tmpCurrency.currencyCode
          ].balanceScanError = tmpBalanceScanError;
          reformatted[tmpCurrency.walletHash][
            tmpCurrency.currencyCode
          ].transactionsScanTime = tmpTransactionsScanTime;
        } else {
          if (
            typeof reformatted[tmpCurrency.walletHash][
              tmpCurrency.currencyCode
            ...
            if (
              typeof alreadyCounted[tmpCurrency.walletHash][
                tmpCurrency.currencyCode
              ][tmpCurrency.address] === 'undefined'
            ) {
              if (
                reformatted[tmpCurrency.walletHash][tmpCurrency.currencyCode]
                  .balanceScanTime *
                ...
                if (tmpCurrency.balanceScanTime * 1 > 0) {
                  reformatted[tmpCurrency.walletHash][
                    tmpCurrency.currencyCode
                  ].balanceScanTime = Math.min(
                    tmpCurrency.balanceScanTime * 1,
                    reformatted[tmpCurrency.walletHash][
                      tmpCurrency.currencyCode
                    ].balanceScanTime * 1
                  );
                ...
                reformatted[tmpCurrency.walletHash][
                  tmpCurrency.currencyCode
                ].balanceScanTime = tmpCurrency.balanceScanTime * 1;
              }
              reformatted[tmpCurrency.walletHash][
                tmpCurrency.currencyCode
              ...
              try {
                ...
                Log.errDaemon(
                  'UpdateAccountListDaemon error on reformatted ' + e.message
                );
              }
              try {
                reformatted[tmpCurrency.walletHash][
                  tmpCurrency.currencyCode
                ].unconfirmed = BlocksoftUtils.add(
                  reformatted[tmpCurrency.walletHash][tmpCurrency.currencyCode]
                    .unconfirmed,
                  tmpCurrency.unconfirmed
                ).toString();
              } catch (e) {
                Log.errDaemon(
                  'UpdateAccountListDaemon error on reformatted2 ' + e.message
                );
              }
              try {
                reformatted[tmpCurrency.walletHash][
                  tmpCurrency.currencyCode
                ].balanceStaked = BlocksoftUtils.add(
                  reformatted[tmpCurrency.walletHash][tmpCurrency.currencyCode]
                    .balanceStaked,
                  tmpCurrency.balanceStaked
                ).toString();
              } catch (e) {
                Log.errDaemon(
                  'UpdateAccountListDaemon error on reformatted3 ' + e.message
                );
              }
              reformatted[tmpCurrency.walletHash][
                tmpCurrency.currencyCode
              ...
              try {
                reformatted[tmpCurrency.walletHash][
                  ...
                  tmpCurrency.address +
                  '/' +
                  tmpCurrency.balanceScanTime +
                  ')';
              } catch (e) {
                Log.errDaemon(
                  'UpdateAccountListDaemon error on balanceAddingLog ' +
                    e.message
                );
              }
            ...
            try {
              ...
              Log.errDaemon(
                'UpdateAccountListDaemon error on balanceAddingLog2 ' +
                  e.message
              );
            }
          }
      ...
      for (tmpWalletHash of tmpWalletHashes) {
        ...
        DaemonCache.CACHE_WALLET_TOTAL.basicCurrencySymbol =
          basicCurrency.symbol || '$';
      ...
      for (tmpCurrency of currentStore.currencyStore.cryptoCurrencies) {
        ...
        let rate = 0;
        if (
          !tmpCurrency.currencyRateJson ||
          typeof tmpCurrency.currencyRateJson[basicCurrencyCode] === 'undefined'
        ) {
        ...
        if (currencyCode === 'BTC') {
          Log.daemon(
            'UpdateAccountListDaemon rate BTC ' +
              rate +
              ' with ' +
              JSON.stringify(basicCurrency)
          );
        }
        ...
        for (tmpWalletHash of tmpWalletHashes) {
          ...
          if (typeof reformatted[tmpWalletHash][currencyCode] === 'undefined') {
            if (
              currencyCode === 'BTC' &&
              typeof walletPub[tmpWalletHash] !== 'undefined'
            ) {
              Log.daemon(
                'UpdateAccountListDaemon need to generate ' +
                  tmpWalletHash +
                  ' ' +
                  currencyCode
              );
              ...
              let accounts = await BlocksoftKeys.discoverAddresses(
                {
                  mnemonic,
                  fullTree: false,
                  fromIndex: 0,
              ...
              if (typeof accounts[currencyCode] === 'undefined') {
                ...
                for (const account of accounts[currencyCode]) {
                  ...
                  if (typeof account.addedData !== 'undefined') {
                    accountJson = Database.escapeString(
                      JSON.stringify(account.addedData)
                    );
                  }
                  const tmp = {
                ...
                if (prepare.length === 0) {
                  if (config.debug.appErrors) {
                    console.log(
                      'UpdateAccountListDaemon skip as no account ' +
                        tmpWalletHash +
                        ' ' +
                        currencyCode +
                        ' fail prepare'
                  ...
                  await Database.setTableName('account')
                    .setInsertData({ insertObjs: prepare })
                    .insert();
                }
              }
              // end low code
          ...
          const accountWallet = allWallets.find(
            (item) => item.walletHash === tmpWalletHash
          );
          ...
          account.feesCurrencySymbol =
            extendedFeesCode.currencySymbol || extendedFeesCode.currencyCode;

          account.feeRates = DaemonCache.getCacheRates(
            account.feesCurrencyCode
          );
          account.walletUseUnconfirmed = accountWallet?.walletUseUnconfirmed;
          try {
            account.balanceRaw = BlocksoftTransferUtils.getBalanceForTransfer({
              balance: account.balance,
              unconfirmed:
                accountWallet && accountWallet?.walletUseUnconfirmed === 1
                  ? account.unconfirmed
                  : false,
            ...
            Log.errDaemon(
              'UpdateAccountListDaemon error on account.balanceRaw ' + e.message
            );
          ...
          if (account.balance > 0) {
            try {
              account.balancePretty = BlocksoftPrettyNumbers.setCurrencyCode(
                currencyCode
              ).makePretty(account.balance, 'updateAccountListDaemon.balance');
            } catch (e) {
              Log.errDaemon(
                'UpdateAccountListDaemon error on account.balance makePretty ' +
                  e.message
              );
            }
          }
          ...
          if (account.unconfirmed > 0) {
            try {
              account.unconfirmedPretty =
                BlocksoftPrettyNumbers.setCurrencyCode(currencyCode).makePretty(
                  account.unconfirmed,
                  'updateAccountListDaemon.unconfirmed'
                );
            } catch (e) {
              Log.errDaemon(
                'UpdateAccountListDaemon error on account.unconfirmed makePretty ' +
                  e.message
              );
            }
          }
          ...
          if (account.balanceStaked > 0) {
            try {
              account.balanceStakedPretty =
                BlocksoftPrettyNumbers.setCurrencyCode(currencyCode).makePretty(
                  account.balanceStaked,
                  'updateAccountListDaemon.balanceStaked'
                );
            } catch (e) {
              Log.errDaemon(
                'UpdateAccountListDaemon error on account.balanceStaked makePretty ' +
                  e.message
              );
            }
          }
          ...
          if (account.balanceRaw > 0) {
            try {
              account.balanceTotalPretty =
                BlocksoftPrettyNumbers.setCurrencyCode(currencyCode).makePretty(
                  account.balanceRaw,
                  'updateAccountListDaemon.balanceRaw'
                );
            } catch (e) {
              Log.errDaemon(
                'UpdateAccountListDaemon error on account.balanceRaw makePretty ' +
                  e.message
              );
            }
          }
          if (rate > 0) {
            ...
            if (rate !== 1) {
              account.basicCurrencyBalanceNorm = BlocksoftUtils.mul(
                account.balancePretty,
                rate
              );
              account.basicCurrencyUnconfirmedNorm = BlocksoftUtils.mul(
                account.unconfirmedPretty,
                rate
              );
              account.basicCurrencyBalanceTotalNorm = BlocksoftUtils.mul(
                account.balanceTotalPretty,
                rate
              );
            }
            account.basicCurrencyBalance = BlocksoftPrettyNumbers.makeCut(
              account.basicCurrencyBalanceNorm,
              2
            ).separated;
            account.basicCurrencyUnconfirmed = BlocksoftPrettyNumbers.makeCut(
              account.basicCurrencyUnconfirmedNorm,
              2
            ).separated;
            account.basicCurrencyBalanceTotal = BlocksoftPrettyNumbers.makeCut(
              account.basicCurrencyBalanceTotalNorm,
              2
            ).separated;
            ...
            str +=
              ' ' +
              account.basicCurrencyBalance +
              ' ' +
              account.basicCurrencyCode +
              ', ';

            const mask = Number(tmpCurrency.isHidden || 0)
              .toString(2)
              .split('')
            ...
            if (accountWallet) {
              if (typeof mask[accountWallet?.walletNumber] === 'undefined') {
                maskedHidden =
                  mask.length === 1 ? mask[mask.length - 1] === '1' : false;
              } else {
            ...
            if (!maskedHidden) {
              if (account.basicCurrencyBalanceNorm > 0) {
                try {
                  DaemonCache.CACHE_WALLET_SUMS[
                    tmpWalletHash
                  ].balanceAddingLog += str;
                  DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].balance =
                    BlocksoftUtils.add(
                      DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].balance,
                      account.basicCurrencyBalanceNorm
                    );
                  DaemonCache.CACHE_WALLET_TOTAL.balance = BlocksoftUtils.add(
                    DaemonCache.CACHE_WALLET_TOTAL.balance,
                    account.basicCurrencyBalanceNorm
                  );
                } catch (e) {
                  Log.errDaemon(
                    'UpdateAccountListDaemon error on sum ' + e.message
                  );
                }
              } else {
                DaemonCache.CACHE_WALLET_SUMS[
              ...
              if (account.basicCurrencyUnconfirmedNorm > 0) {
                try {
                  DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].unconfirmed =
                    BlocksoftUtils.add(
                      DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].unconfirmed,
                      account.basicCurrencyUnconfirmedNorm
                    );
                  DaemonCache.CACHE_WALLET_TOTAL.unconfirmed =
                    BlocksoftUtils.add(
                      DaemonCache.CACHE_WALLET_TOTAL.unconfirmed,
                      account.basicCurrencyUnconfirmedNorm
                    );
                  ...
                  Log.errDaemon(
                    'UpdateAccountListDaemon error on sum2 ' + e.message
                  );
                }
              }
            } else {
          ...
          DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].balance =
            BlocksoftPrettyNumbers.makeCut(
              DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].balance,
              2
            ).justCutted;
          DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].unconfirmed =
        ...
        DaemonCache.CACHE_WALLET_TOTAL.balance = BlocksoftPrettyNumbers.makeCut(
          DaemonCache.CACHE_WALLET_TOTAL.balance,
          2
        ).justCutted;
        DaemonCache.CACHE_WALLET_TOTAL.unconfirmed =
      ...
      for (const tmpWalletHash in DaemonCache.CACHE_WALLET_SUMS) {
        const cacheBalanceString =
          DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].balanceAddingLog +
          DaemonCache.CACHE_WALLET_SUMS[tmpWalletHash].balanceAddingLogHidden;
      ...
      walletActions.setWalletsGeneralData(
        DaemonCache.CACHE_WALLET_TOTAL.balance,
        DaemonCache.CACHE_WALLET_TOTAL.basicCurrencySymbol
      );
      ...
      try {
        if (
          ...
          if (
            typeof DaemonCache.CACHE_ALL_ACCOUNTS[selectedAccount.walletHash][
              selectedAccount.currencyCode
            ] === 'undefined'
          ) {
            Log.daemon(
              'UpdateAccountListDaemon error when no selected ' +
                selectedAccount.currencyCode
            );
          } else {
            if (
              DaemonCache.CACHE_ALL_ACCOUNTS[selectedAccount.walletHash][
                selectedAccount.currencyCode
        ...
        Log.errDaemon(
          'UpdateAccountListDaemon error on setSelected ' + e.message
        );
      }

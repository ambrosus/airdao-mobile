...
import {
  getFioObtData,
  resolveCryptoCodes
} from '../../../crypto/blockchains/fio/FioUtils.jts';
...
class UpdateAccountBalanceAndTransactions {
  ...
  updateAccountBalanceAndTransactions = async (callParams) => {
    ...
    if (!force || source === 'BACK') {
      ...
      if (setting === 'none') {
        ...
        if (diff < CACHE_VALID_10MIN_TIME) {
          Log.daemon(
            'UpdateAccountBalanceAndTransactions skipped by diff ' + diff
          );
          return false;
        }
      }
    ...
    try {
      ...
      if (!allWallets && source !== 'BACK') {
        params.walletHash = await settingsActions.getSelectedWallet(
          'UpdateAccountBalanceAndTransactions'
        );
      }

      tmpAction = 'params init';
      if (
        typeof callParams !== 'undefined' &&
        callParams &&
        typeof callParams.currencyCode !== 'undefined'
      ) {
        if (force) {
      ...
      let accounts = await accountScanningDS.getAccountsForScan({
        ...params,
        force: false
      });
      ...
      for (account of accounts) {
        Log.daemon(
          'UpdateAccountBalanceAndTransactions called - todo account ' +
            account.id +
            ' ' +
      ...
      for (account of accounts) {
        ...
        if (
          typeof CACHE_ONE_ACCOUNTS[
            account.currencyCode + '_' + account.address
          ] !== 'undefined'
        ) {
          continue;
        }
        ...
        if (typeof CACHE_CUSTOM_TIME[account.currencyCode] !== 'undefined') {
          ...
          if (
            await this._accountRun(
              ...
              source,
              running > 0
                ? CACHE_CUSTOM_TIME[account.currencyCode]
                : CACHE_VALID_TIME,
              force,
              onlyBalances
            )
      ...
      if (shouldUpdateBalance) {
        await UpdateAccountListDaemon.updateAccountListDaemon({
          force: true,
          source: 'SHOULD_UPDATE_BALANCE'
        });
      }
    } catch (e) {
      if (config.debug.appErrors) {
        console.log(
          'UpdateAccountBalanceAndTransactions balance error ' +
            source +
            ' ' +
            e.message,
          e
  ...
  loadFioData = async (currencyCode) => {
    ...
    Log.daemon(
      'UpdateAccountBalanceAndTransactions loadFioData ' + currencyCode
    );
    try {
      ...
      if (config.debug.cryptoErrors) {
        console.log(
          'UpdateAccountBalanceAndTransactions error on loadFioData ' +
            e.message,
          e
        );
      }
      Log.errDaemon(
        'UpdateAccountBalanceAndTransactions error on loadFioData ' + e.message
      );
    }
  };
  ...
  async _accountRun(
    ...
    Log.daemon(
      'UpdateAccountBalanceAndTransactions _accountRun init ' +
        account.id +
        ' ' +
        account.currencyCode +
        ' ' +
    ...
    if (
      account.accountJson &&
      typeof account.accountJson.addressHex !== 'undefined'
    ) {
      addressToScan = account.accountJson.addressHex;
      Log.daemon(
        'UpdateAccountBalanceAndTransactions changing address ' +
          account.currencyCode +
          ' ' +
          account.address +
          ' => ' +
    ...
    if (
      !force &&
      typeof CACHE_SCANNING[account.currencyCode + ' ' + addressToScan] !==
        'undefined'
    ) {
      const diff =
        now - CACHE_SCANNING[account.currencyCode + ' ' + addressToScan];
      if (diff < time) {
        Log.daemon(
          'UpdateAccountBalanceAndTransactions skipped as running ' +
            account.currencyCode +
            ' ' +
            account.address +
            ' => diff:' +
    ...
    try {
      Log.daemon(
        'UpdateAccountBalanceAndTransactions newBalance ' +
          account.currencyCode +
          ' ' +
          addressToScan
      );
      ...
      if (account.currencyCode === 'BTC') {
        ...
        if (
          account.walletIsHd &&
          account.derivationPath !== 'm/49quote/0quote/0/1/0'
        ) {
          ...
          newBalance = await BlocksoftBalances.setCurrencyCode(
            account.currencyCode
          )
            .setAddress(addressToScan)
            .setAdditional(additional)
            .setWalletHash(account.walletHash)
            .getBalance('AccountRunBalancesBtc');
        }
      } else {
        newBalance = await BlocksoftBalances.setCurrencyCode(
          account.currencyCode
          ...
          .setAddress(addressToScan)
          .setAdditional(account.accountJson)
          .setWalletHash(account.walletHash)
          .getBalance('AccountRunBalances');
      }
      if (!newBalance || typeof newBalance.balance === 'undefined') {
        if (account.balanceScanBlock === 0 && account.balanceScanTime === 0) {
          updateObj.balanceScanLog =
            account.address +
            ' empty response, old balance ' +
            account.balance +
            ', ' +
        ...
        balanceError =
          ...
          JSON.stringify(newBalance);
        Log.daemon(
          'UpdateAccountBalanceAndTransactions newBalance something wrong ' +
            account.currencyCode +
            ' ' +
            addressToScan +
            ' => ' +
      ...
      if (config.debug.appErrors) {
        console.log(
          'UpdateAccountBalanceAndTransactions newBalance from ' +
            source +
            ' loaded ' +
            account.currencyCode +
            ' ' +
      ...
      if (newBalance && typeof newBalance.balance !== 'undefined') {
        if (typeof account.balance === 'undefined') {
          ...
          if (typeof account.balanceStaked === 'undefined') {
            ...
            newBalance.balanceStaked * 1 !==
            account.balanceStaked * 1
          ) {
            // toString here somehow do undefined sometimes
            shouldUpdateBalance = true;
          }
        }

        if (
          typeof newBalance.balanceScanBlock !== 'undefined' &&
          (typeof account.balanceScanBlock === 'undefined' ||
            newBalance.balanceScanBlock * 1 < account.balanceScanBlock * 1)
        ) {
          continueWithTx = false;
          updateObj.balanceProvider = newBalance.provider;
          updateObj.balanceScanLog =
            account.address +
            ' block error, ignored new ' +
            newBalance.balance +
            ' block ' +
            newBalance.balanceScanBlock +
            ', old balance ' +
            account.balance +
            ' block ' +
            account.balanceScanBlock;
          ...
          logData.addressShort = account.address
          ...
          MarketingEvent.setBalance(
            logData.walletHash,
            logData.currencyCode,
            logData.newBalance,
            logData
          );
        } else {
        ...
        Log.daemon(
          'UpdateAccountBalanceAndTransactions newBalance ok Prepared ' +
            account.currencyCode +
            ' ' +
            account.address +
            ' new balance ' +
        ...
        updateObj.balanceScanLog =
          account.address +
          ' no balance, old balance ' +
          account.balance +
          ', ' +
          balanceError;
        updateObj.balanceScanError = 'account.balanceBadNetwork';
        Log.daemon(
          'UpdateAccountBalanceAndTransactions newBalance not Prepared ' +
            account.currencyCode +
            ' ' +
            account.address +
            ' old balance ' +
    ...
    try {
      updateObj.balanceScanLog =
        ...
        ' ' +
        updateObj.balanceScanLog.substr(0, 1000);
      await accountBalanceDS.updateAccountBalance({ updateObj }, account);
    } catch (e) {
    ...
    try {
      ...
      try {
        Log.daemon(
          'UpdateAccountBalanceAndTransactions newTransactions ' +
            account.currencyCode +
            ' ' +
            account.address
        );
        if (account.currencyCode === 'BTC' || account.currencyCode === 'LTC') {
          ...
          newTransactions = await BlocksoftTransactions.getTransactions(
            { account, additional },
            'AccountRunTransactionsBtc'
          );
        } else {
          newTransactions = await BlocksoftTransactions.getTransactions(
            { account, additional: account.accountJson },
            'AccountRunTransactions'
          );
        }
        ...
        if (config.debug.appErrors) {
          console.log(
            'UpdateAccountBalanceAndTransactions newTransactions something wrong ' +
              account.currencyCode +
              ' ' +
              account.address +
              ' => transactionsError ' +
      ...
      Log.daemon(
        'UpdateAccountBalanceAndTransactions newTransactions loaded ' +
          account.currencyCode +
          ' ' +
          addressToScan
      );
      ...
      try {
        transactionUpdateObj = await AccountTransactionsRecheck(
          newTransactions,
          account,
          'RECHECK ' + source
        );
      ...
      try {
        transactionUpdateObj.transactionsScanLog =
          new Date().toISOString() +
          ' ' +
          transactionsError +
          ', ' +
        ...
        if (account.transactionsScanLog) {
          transactionUpdateObj.transactionsScanLog +=
            ' ' + account.transactionsScanLog;
        }
        await accountDS.updateAccount(
          { updateObj: transactionUpdateObj },
          account
        );
      } catch (e) {
    ...
    if (account.currencyCode === 'TRX') {
      ...
      for (const sub of accounts) {
        if (
          sub.currencyCode !== 'TRX_USDT' &&
          sub.currencyCode.indexOf('TRX_') === 0
        ) {
          if (
            await this._accountRun(
              sub,
    ...
    Log.daemon(
      'UpdateAccountBalanceAndTransactions _accountRun finish ' +
        account.id +
        ' ' +
        account.currencyCode +
        ' ' +

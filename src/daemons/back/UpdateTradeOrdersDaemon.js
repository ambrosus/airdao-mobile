...
class UpdateTradeOrdersDaemon {
  ...
  fromApi = async (walletHash, orderHash) => {
    ...
    if (typeof CACHE_ONE_ORDER[orderHash] !== 'undefined') {
      ...
      if (diff < CACHE_VALID_TIME) {
        Log.daemon(
          'UpdateTradeOrders.fromApi ' + orderHash + ' skipped by diff ' + diff
        );
    ...
    try {
      ...
      if (
        typeof tmpTradeOrdersV3 !== 'undefined' &&
        tmpTradeOrdersV3 &&
        tmpTradeOrdersV3.length > 0
      ) {
      ...
      if (config.debug.appErrors) {
        console.log(
          'UpdateTradeOrdersDaemon.fromApi error ' + e.message + ' tmpOneOrder'
        );
      }
  ...
  removeId = async (removeId) => {
    ...
    try {
      ...
      const found = await Database.query(
        ` SELECT id, hidden_at FROM transactions WHERE bse_order_id = '${removeId}' `,
        true
      );
      ...
      const { transactionsToView } =
        store.getState().mainStore.selectedAccountTransactions;
      if (account) {
        let found = false;
        ...
        if (transactionsToView) {
          for (const transaction of transactionsToView) {
            if (
              transaction.bseOrderData &&
              typeof transaction.bseOrderData.orderId !== 'undefined' &&
              transaction.bseOrderData.orderId == removeId
            ) {
              found = true;
            } else {
      ...
      Log.errDaemon(
        'UpdateTradeOrders removeId ' + removeId + ' error ' + e.message
      );
    }
  };
  ...
  updateTradeOrdersDaemon = async (params, dataUpdate = false) => {
    if (
      typeof params.source !== 'undefined' &&
      params.source === 'ACCOUNT_OPEN' &&
      CACHE_LAST_TIME
    ) {
      const now = new Date().getTime();
    ...
    try {
      ...
      if (!dataUpdate) {
        if (config.debug.appErrors) {
          console.log(
            new Date().toISOString() + ' UpdateTradeOrdersDaemon loading new'
          );
        }
        asked = true;
        res = await ApiProxy.getAll({
          source: 'UpdateTradeOrdersDaemon.updateTradeOrders'
        });
        if (config.debug.appErrors) {
          console.log(
            new Date().toISOString() +
              ' UpdateTradeOrdersDaemon loaded new finished'
          );
        }
      } else {
        res = dataUpdate;
      ...
      const tmpTradeOrders =
        typeof res !== 'undefined' && typeof res.cbOrders !== 'undefined'
          ? res.cbOrders
          : false;
      ...
      try {
        if (
          typeof tmpTradeOrders === 'undefined' ||
          !tmpTradeOrders ||
          !tmpTradeOrders.length
        ) {
          return false;
        }
        ...
        if (!asked) {
          if (config.debug.appErrors) {
            console.log(
              new Date().toISOString() + ' UpdateTradeOrdersDaemon loaded proxy'
            );
          }
        }
          ...
          try {
            ...
            if (item.exchangeWayType !== 'BUY') {
              tmps.push({
                ...
                addressTo:
                  typeof item.depositAddress !== 'undefined'
                    ? item.depositAddress
                    : false,
            ...
            if (item.exchangeWayType !== 'SELL') {
              tmps.push({
                ...
                addressTo:
                  typeof item.outDestination !== 'undefined' &&
                  item.outDestination &&
                  item.outDestination.toString().indexOf('*') === -1
                    ? item.outDestination
                    : false,
            ...
            for (const tmp of tmps) {
              ...
              if (
                ...
                found.array[0].transaction_hash === ''
              ) {
                if (
                  typeof item.searchHashByAmount !== 'undefined' &&
                  item.searchHashByAmount &&
                  noHash
                ) {
                  Log.daemon(
                    'UpdateTradeOrders called inner search ' +
                      JSON.stringify(item)
                  );
                  let sql2 = '';
                  const rawAmount = BlocksoftPrettyNumbers.setCurrencyCode(
                    tmp.currencyCode
                  ).makeUnPretty(tmp.addressAmount);
                  if (typeof tmp.addressTo !== 'undefined' && tmp.addressTo) {
                    sql2 = `
                                             ...
                                             WHERE transaction_hash != '' AND (bse_order_id IS NULL OR bse_order_id='') AND currency_code='${
                                               tmp.currencyCode
                                             }'
                  ...
                  if (sql2) {
                    ...
                    if (found2 && found2.array) {
                      for (const found22 of found2.array) {
                        ...
                        if (Math.abs(createdAt - item.createdAt) > 12000000)
                          continue; //60 * 1000 * 200 minutes
              ...
              if (found && found.array && found.array.length > 0) {
                ...
                for (const row of found.array) {
                  ...
                  if (!(row.bse_order_data === escaped)) {
                    ...
                    if (
                      scanLog.indexOf(` UPDATED ORDER ${item.orderId} `) === -1
                    ) {
                ...
                if (toRemove.length > 0) {
                  const sql3 = ` DELETE FROM transactions WHERE id IN (${toRemove.join(
                    ','
                  )}) AND id != ${id} `;
            ...
            for (const tmp of tmps) {
              ...
              const sql = `
                                            ...
                                            address_amount, address_to, bse_order_id, bse_order_id_${
                                              tmp.suffix
                                            }, bse_order_data)
                                            VALUES ('${currencyCode}', '${walletHash}', '0', '', '${
                tmp.updateHash ? tmp.updateHash : ''
              }', '', 'FROM ORDER ${createdAt} ${item.orderId}', '${createdAt}',
                                            '${tmp.addressAmount}', '', '${
                item.orderId
              }', '${item.orderId}', '${Database.escapeString(
                JSON.stringify(item)
              )}')
                                       `;
              await Database.query(sql, true);
            ...
            if (config.debug.appErrors) {
              console.log(
                'UpdateTradeOrders one order error ' + e.message,
        ...
        if (config.debug.appErrors) {
          console.log(
            'UpdateTradeOrders all orders error ' + e.message,
            e,
            JSON.parse(JSON.stringify(tmpTradeOrders))
          );
        }
        throw new Error(
          'UpdateTradeOrders all orders error ' +
            e.message +
            ' ' +
            JSON.stringify(tmpTradeOrders)
        );
      }
    } catch (e) {
      if (config.debug.appErrors) {
      ...
      if (Log.isNetworkError(e.message) && TRY_COUNTER < 10) {
        TRY_COUNTER++;
        Log.daemon(
          'UpdateTradeOrders network try ' + TRY_COUNTER + ' ' + e.message
        );

...
import BlocksoftFixBalance from '@crypto/common/AirDAOFixBalance';
...
class DaemonCache {
  ...
  async getCacheAccount(walletHash, currencyCode) {
    ...
    if (
      typeof this.CACHE_ALL_ACCOUNTS[walletHash][currencyCode] === 'undefined'
    ) {
      return this._getFromDB(walletHash, currencyCode);
    }
  ...
  getCacheAccountStatic(walletHash, currencyCode) {
    if (
      typeof this.CACHE_ALL_ACCOUNTS[walletHash] === 'undefined' ||
      typeof this.CACHE_ALL_ACCOUNTS[walletHash][currencyCode] === 'undefined'
    ) {
      return { balance: '0' };
    }

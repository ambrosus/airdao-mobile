...
} from './SendActionsBlockchainWrapper'; // TODO
...
const findWalletPlus = function (currencyCode: string): {
  wallet: any;
  cryptoCurrency: any;
  account: any;
} {
  ...
  if (cryptoCurrency.currencyCode) {
    // @ts-ignore
    account =
      accountList[selectedWallet.walletHash][cryptoCurrency.currencyCode];
  }
...
const formatDict = async function (cryptoCurrency: any, account: any) {
  const dict = {
    ...
    feesBasicCurrencyRate:
      account.feeRates?.basicCurrencyRate || account.basicCurrencyRate,
    feesBasicCurrencySymbol:
      account.feeRates?.basicCurrencySymbol || account.basicCurrencySymbol,
    feesCurrencyCode: account.feesCurrencyCode,
    feesCurrencySymbol: account.feesCurrencySymbol
  };
  if (CACHE_SEND_INPUT_TYPE === 'none') {
    const inputType = await Cache.getItem(CacheKey.SendInputType);
    CACHE_SEND_INPUT_TYPE = inputType !== 'CRYPTO' ? 'FIAT' : 'CRYPTO';
  }
...
export namespace SendActionsStart {
  ...
  export const startFromWalletConnect = async (
    data: {
      currencyCode: string;
    ...
    try {
      ...
      if (typeof account.derivationPath === 'undefined') {
        throw new Error(
          'SendActionsStart.startFromWalletConnect required account.derivationPath'
        );
      }
      if (
        typeof data.walletConnectData.value !== 'undefined' &&
        data.walletConnectData.value &&
        data.walletConnectData.value.toString().indexOf('0x') === 0
      ) {
        data.walletConnectData.value = BlocksoftUtils.decimalToHexWalletConnect(
          data.walletConnectData.value
        );
      }
      ...
      const ui = {
        uiType: uiType,
        cryptoValue: data.walletConnectData.value
          ? data.walletConnectData.value
          : 0,
        ...
        transactionFilterType:
          data.transactionFilterType || TransactionFilterTypeDict.WALLET_CONNECT
      };
  ...
  export const startFromAccountScreen = async (
    currencyCode: string,
  ...
  export const startFromHomeScreen = async (
    cryptoCurrency: any,
    account: any,
    uiType = 'HOME_SCREEN'
  ) => {
  ...
  export const startFromDEX = async (
    data: {
      addressTo: string;
      amount: string;
      currencyCode: string;
  ...
  export const getTransferAllBalanceFromBSE = async (data: {
    ...
    // @ts-ignore
    const addressToForTransferAll =
      BlocksoftTransferUtils.getAddressToForTransferAll(data);
    const { cryptoCurrency, account } = findWalletPlus(data.currencyCode);
    const dict = await formatDict(cryptoCurrency, account);
    SendActionsBlockchainWrapper.beforeRender(cryptoCurrency, account, {
  ...
  export const startFromBSE = async (
    data: {
      amount: string;
      address: string;
    ...
    const addressTo =
      typeof data.addressTo !== 'undefined' ? data.addressTo : data.address;
    const { cryptoCurrency, account } = findWalletPlus(data.currencyCode);
    const dict = await formatDict(cryptoCurrency, account);
    const amount = AirDAOPrettyNumbers.setCurrencyCode(
      data.currencyCode
    ).makeUnPretty(data.amount);
    SendActionsBlockchainWrapper.beforeRender(cryptoCurrency, account, {
      addressTo,
      amount: amount,
      memo: data.memo
  ...
  export const startFromTransactionScreenBoost = async (
    accountSelected: any,
    transaction: any
  ) => {
    const { cryptoCurrency, account } = findWalletPlus(
      accountSelected.currencyCode
    );
    if (typeof account.derivationPath === 'undefined') {
      throw new Error(
        'SendActionsStart.startFromTransactionScreenBoost required account.derivationPath'
      );
    }
    const dict = await formatDict(cryptoCurrency, account);
  ...
  export const startFromTransactionScreenRemove = async (
    accountSelected: any,
    transaction: any
  ) => {
    const { cryptoCurrency, account } = findWalletPlus(
    ...
    if (
      ...
      if (
        typeof transaction.transactionJson.comment !== 'undefined' &&
        transaction.transactionJson.comment
      ) {
        ui.comment = transaction.transactionJson.comment;
      }
    }
  ...
  export const startFromDeepLinking = async (
    data: {
    ...
    const amountRaw = AirDAOPrettyNumbers.setCurrencyCode(
      data.currencyCode
    ).makeUnPretty(amount);
    ...
    if (
      typeof data.needToDisable !== 'undefined' &&
      data.needToDisable &&
      addressTo &&
      addressTo !== '' &&
      amount &&
  ...
  export const startFromQRCodeScanner = async (
    data: any,
    uiType = 'MAIN_SCANNER'
  ) => {
  ...
  export const startFromFioRequest = async (
    currencyCode: any,
    fioRequestDetails: {
    ...
    const amountRaw =
      AirDAOPrettyNumbers.setCurrencyCode(currencyCode).makeUnPretty(amount);

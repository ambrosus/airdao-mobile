...
export namespace SendActionsBlockchainWrapper {
  export const beforeRender = (
    ...
    if (
      typeof additional.tbk !== 'undefined' &&
      additional.tbk &&
      additional.tbk.transactionAction
    ) {
      if (additional.tbk.transactionAction === 'transactionSpeedUp') {
        newCountedFeesData.transactionSpeedUp =
          additional.tbk.transactionBoost.transactionHash;
      } else if (
        additional.tbk.transactionAction === 'transactionReplaceByFee'
      ) {
        newCountedFeesData.transactionReplaceByFee =
          additional.tbk.transactionBoost.transactionHash;
      } else if (
        additional.tbk.transactionAction === 'transactionRemoveByFee'
      ) {
        newCountedFeesData.transactionReplaceByFee =
          additional.tbk.transactionBoost.transactionHash;
      } else {
        throw new Error(
          'undefined SendActionsBlockchainWrapper beforeRender transactionAction ' +
            additional.tbk.transactionAction
        );
      }
      if (
        typeof additional.tbk.transactionBoost.transactionJson !==
          'undefined' &&
        additional.tbk.transactionBoost.transactionJson
      ) {
        newCountedFeesData.transactionJson =
          additional.tbk.transactionBoost.transactionJson;
      }
    }
    ...
    if (
      ...
      JSON.stringify(newCountedFeesData)
    ) {
      return;
    }
    if (
      newCountedFeesData.addressFrom !== CACHE_DATA.countedFeesData.addressFrom
    ) {
  ...
  export const getCustomFeeRate = async (newFee: any) => {
    ...
    try {
      ...
      if (
        typeof countedFees.selectedFeeIndex !== 'undefined' &&
        countedFees.selectedFeeIndex >= 0
      ) {
        // @ts-ignore
      ...
      if (config.debug.appErrors) {
        console.log(
          'SendActionsBlockchainWrapper.getCustomFeeRate error ' + e.message
        );
      }
      if (e.message.indexOf('SERVER_RESPONSE_') !== -1) {
        const extend = BlocksoftDict.getCurrencyAllSettings(
          newCountedFeesData?.currencyCode
        );
        Log.errorTranslate(
          e,
          'SendActionsBlockchainWrapper.getCustomFeeRate',
          extend
        );
        showModal({
          type: 'INFO_MODAL',
  ...
  export const getFeeRate = async (
    uiData = {},
    precountedSelectedFee = false
  ) => {
    let newCountedFeesData = {};
    try {
      if (
        typeof uiData === 'undefined' ||
        typeof uiData.addressTo === 'undefined'
      ) {
        uiData = store.getState().sendScreenStore.ui;
      }

      const forceExecAmount =
        typeof uiData.bse !== 'undefined' &&
        typeof uiData.bse.forceExecAmount !== 'undefined' &&
        uiData.bse.forceExecAmount;
      ...
      if (!store.getState().sendScreenStore.fromBlockchain.neverCounted) {
        if (
          ...
          return {
            isTransferAll: newCountedFeesData.isTransferAll,
            amount: newCountedFeesData.amount,
            source: 'CACHE_COUNTED',
      ...
      if (precountedSelectedFee) {
        Log.log(
          'SendActionsBlockchainWrapper.getFeeRate has precountedSelectedFee ',
          precountedSelectedFee
        );
        countedFees = {
      ...
      if (
        forceExecAmount &&
        typeof countedFees !== 'undefined' &&
        countedFees &&
        typeof countedFees.fees !== 'undefined' &&
        countedFees.fees
      ) {
      ...
      if (
        typeof countedFees.additionalData !== 'undefined' &&
        countedFees.additionalData
      ) {
      ...
      if (config.debug.appErrors) {
        console.log(
          'SendActionsBlockchainWrapper.getFeeRate error ' + e.message,
          e
        );
      }
      if (typeof e.message === 'undefined') {
        ...
        Log.errorTranslate(
          e,
          'SendActionsBlockchainWrapper.getFeeRate',
          extend
        );
        showModal({
          type: 'INFO_MODAL',
        ...
        Log.log(
          'SendActionsBlockchainWrapper.getFeeRate inner error ' + e.message
        );
      }
    }
  ...
  export const getTransferAllBalance = async (uiData = {}) => {
    ...
    try {
      ...
      newCountedFeesData.isTransferAll = uiData.isTransferAll;
      if (
        !newCountedFeesData.addressTo ||
        newCountedFeesData.addressTo === '' ||
        newCountedFeesData.addressTo === '?'
      ) {
        newCountedFeesData.addressTo =
          BlocksoftTransferUtils.getAddressToForTransferAll({
            currencyCode: newCountedFeesData.currencyCode,
            address: newCountedFeesData.addressFrom
          });
      }
      if (
        !store.getState().sendScreenStore.fromBlockchain.neverCounted &&
        JSON.stringify(CACHE_DATA.countedFeesData) ===
          JSON.stringify(newCountedFeesData)
      ) {
        return {
          transferAllBalance: CACHE_DATA.transferAllBalance,
          source: 'CACHE_COUNTED',
          addressTo: newCountedFeesData.addressTo
        };
      }
      ...
      return {
        transferAllBalance:
          typeof transferAllBalance !== 'undefined' && transferAllBalance
            ? transferAllBalance
            : 0,
        source: 'NEW_COUNTED',
        addressTo: newCountedFeesData.addressTo,
      ...
      if (config.debug.appErrors) {
        console.log(
          'SendActionsBlockchainWrapper.getTransferAllBalance error ' +
            e.message
        );
      }
      if (typeof e.message === 'undefined') {
        Log.log(
          'SendActionsBlockchainWrapper.getTransferAllBalance strange error'
        );
      } else if (e.message.indexOf('SERVER_RESPONSE_') !== -1) {
        const extend = BlocksoftDict.getCurrencyAllSettings(
          newCountedFeesData?.currencyCode
        ...
        Log.errorTranslate(
          e,
          'SendActionsBlockchainWrapper.getTransferAllBalance ',
          extend
        );
        showModal({
          type: 'INFO_MODAL',
        ...
        Log.log(
          'SendActionsBlockchainWrapper.getTransferAllBalance inner error ' +
            e.message
        );
      }
    }
  ...
  export const actualSend = async (
    sendScreenStore: any,
    uiErrorConfirmed: any,
    selectedFee: any,
    newCryptoValue: any
  ) => {
    const newCountedFeesData = { ...CACHE_DATA.countedFeesData };
